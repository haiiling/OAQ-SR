...
class RDB_Conv(nn.Module):
    def __init__(self, args, inChannels, growRate, kSize=3):
        ...
        self.conv = nn.Sequential(*[
            quantize.QConv2d(args, Cin, G, kSize, stride=1, padding=(kSize-1)//2, bias=True, dilation=1, groups=1),
            nn.ReLU()
        ])
    ...
    def forward(self, x):
        ...
        out, bit, err = self.conv[0]([out, bit])
...
class RDB(nn.Module):
    def __init__(self, args, growRate0, growRate, nConvLayers, kSize=3):
        ...
        self.LFF = quantize.QConv2d(args, G0 + C*G, G0, 1, padding=0, stride=1, bias=True, to_8bit=False) # 1x1 conv is non-adaptively quantized
        self.args = args
...
class RDN(nn.Module):
    def __init__(self, args):
        ...
        if args.fq:
            self.SFENet1 = quantize.QConv2d(args, args.n_colors, G0, kSize, padding=(kSize-1)//2, stride=1, bias=True, to_8bit=True)
            self.SFENet2 = quantize.QConv2d(args, G0, G0, kSize, padding=(kSize-1)//2, stride=1, bias=True, to_8bit=True)
        else:
        ...
        if args.fq:
            if r == 2 or r == 3:
                ...
                self.UPNet = nn.Sequential(*[
                    quantize.QConv2d(args, G0, self.G * 4, kSize, padding=(kSize-1)//2, stride=1, bias=True, to_8bit=True),
                    nn.PixelShuffle(2),
                    quantize.QConv2d(args, self.G, self.G * 4, kSize, padding=(kSize-1)//2, stride=1, bias=True, to_8bit=True),
                    nn.PixelShuffle(2),
                    quantize.QConv2d(args, self.G, args.n_colors, kSize, padding=(kSize-1)//2, stride=1, bias=True, to_8bit=True)
                ])
            else:
    ...
    def forward(self, x):
        ...
        if self.args.fq:
            f__1, bit_fq, err = self.SFENet1([x, bit_fq])
            x, bit_fq, err  = self.SFENet2([f__1, bit_fq])
        else:
            f__1 = self.SFENet1(x)
            x  = self.SFENet2(f__1)
        ...
        x, bit, err = self.GFF[0]([torch.cat(RDBs_out,1), bit])
        x, bit, err = self.GFF[1]([x, bit])

        x += f__1
    
        if self.args.fq:
            out, bit_fq, err = self.UPNet[0]([x, bit_fq])
            out = self.UPNet[1](out)
            out, bit_fq, err = self.UPNet[2]([out, bit_fq])
            if len(self.UPNet) > 3:
                out = self.UPNet[3](out)
                out, bit_fq, err = self.UPNet[4]([out, bit_fq])
        else:
            out = self.UPNet(x)

        return out, feat, bit, err

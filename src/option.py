...
parser.add_argument('--batch_size_update', type=int, default=6, help='input batch size for update')
...
parser.add_argument('--count_std', action='store_true',
                    help='measure the per-layer quantization sensitivity s_k during calibration (Eq. 4)')
parser.add_argument('--saft', action='store_true',
                    help='weight the feature loss by s_k, i.e. sensitivity-aware finetuning (Eq. 6)')
parser.add_argument('--fine_tuning', action='store_true', help='fine-tuning')
...
parser.add_argument('--lr_bp', type=float, default=0.05, help='learning rate')
...
parser.add_argument('--w_sktloss', type=float, default=10.0,
                    help='scale of the sensitivity-aware loss L_sen')
parser.add_argument('--lambda_rec', type=float, default=5.0,
                    help='lambda: weight of the reconstruction loss L_rec in Eq. 7')
...
parser.add_argument('--quantizer', default='minmax', choices=('minmax', 'percentile', 'omse', 'entropy'), help='quantizer to use')
parser.add_argument('--quantizer_w', default='minmax', choices=('minmax', 'percentile', 'omse'), help='quantizer to use')
parser.add_argument('--bp_init', default='asym',
                    choices=('norm', 'laplace', 'search', 'coarse2fine', 'asym'),
                    help='how the breakpoint bp is initialised from the calibration statistics: '
                         "'asym' uses the 99th percentile of the activations (paper default); "
                         "'norm'/'laplace' use the closed-form approximation for the matching "
                         "distribution; 'search'/'coarse2fine' search for the error-minimising bp")
...
parser.add_argument('--bac_beta', type=float, default=0.5, help='beta for EMA in BaC') # Bit aware clipping
